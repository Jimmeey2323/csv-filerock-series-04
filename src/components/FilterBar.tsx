...
const FilterBar: React.FC<FilterBarProps> = ({
  ...
  return <Card className="bg-white/70 backdrop-blur-sm rounded-lg border shadow-sm mb-4 animate-fade-in">
        ...
        <div className="flex flex-col space-y-4">
          ...
          <div className="flex justify-between items-center border-b pb-3">
            <div className="flex items-center space-x-2">
              ...
              <Switch id="data-view-toggle" checked={activeDataMode === 'studio'} onCheckedChange={checked => onDataModeChange(checked ? 'studio' : 'teacher')} />
          ...
          {activeFiltersCount > 0 && <div className="flex flex-wrap gap-2 mb-2">
              <div className="text-sm text-muted-foreground flex items-center mr-2">
                <Filter className="h-3.5 w-3.5 mr-1" /> Active filters:
              </div>
              {filters.location && <Badge variant="active" className="flex items-center gap-1 px-3 py-1">
                  <MapPin className="h-3 w-3" />
                  {filters.location === 'all-locations' ? 'All Locations' : filters.location}
                </Badge>}
              {filters.teacher && <Badge variant="active" className="flex items-center gap-1 px-3 py-1">
                  <User className="h-3 w-3" />
                  {filters.teacher === 'all-teachers' ? 'All Teachers' : filters.teacher}
                </Badge>}
              {filters.period && <Badge variant="active" className="flex items-center gap-1 px-3 py-1">
                  <Calendar className="h-3 w-3" />
                  {filters.period === 'all-periods' ? 'All Periods' : filters.period}
                </Badge>}
              {filters.search && <Badge variant="active" className="flex items-center gap-1 px-3 py-1">
          ...
          <div className="flex flex-col md:flex-row gap-3">
            <div className="relative flex-1">
              <CommandSearchInput placeholder="Search teacher..." value={filters.search} onChange={value => handleFilterChange('search', value)} className="flex-1" />
            </div>
                    ...
                    <div>
                      <Select value={filters.period} onValueChange={value => handleFilterChange('period', value)}>
                        ...
                        <SelectContent>
                          ...
                          {periods.map(period => <SelectItem key={period} value={period}>
                              {period}
                            </SelectItem>)}
                        </SelectContent>
                      </Select>
            ...
            <div className="flex gap-2">
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button variant="outline" className="shrink-0 bg-white hover:bg-secondary/50" onClick={handleReset}>
