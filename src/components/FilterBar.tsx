...
const FilterBar: React.FC<FilterBarProps> = ({
  ...
  const quickPeriods = [
    { label: 'All Time', value: 'all-time', icon: Clock },
    ...
    { label: 'Q2 2023', value: 'q2-2023', icon: BarChart3 }
  ];
  ...
  const handleQuickPeriodClick = (period: { label: string; value: string }) => {
      ...
      <CardContent className="p-8">
        <div className="flex flex-col space-y-8">
            ...
            <div className="flex items-center space-x-4">
              <div className="bg-gradient-to-r from-slate-100/80 to-blue-50/60 p-1.5 rounded-2xl flex items-center backdrop-blur-sm border border-white/30 shadow-lg">
                <Label 
                  ...
                  className={`px-4 py-2.5 rounded-xl text-sm font-semibold cursor-pointer transition-all duration-500 flex items-center gap-2 ${
              ...
              <Popover open={viewOptionsOpen} onOpenChange={setViewOptionsOpen}>
                <PopoverTrigger asChild>
                  <Button variant="outline" size="sm" className="flex items-center gap-2 bg-white/60 backdrop-blur-sm border-white/40 hover:bg-white/80 transition-all duration-300 shadow-lg hover:shadow-xl rounded-xl">
              ...
              {quickPeriods.map(period => {
                ...
                return (
                  <Button
                    ...
                    className={cn(
                      "whitespace-nowrap transition-all duration-500 rounded-xl flex items-center gap-2 min-w-fit",
                      "hover:scale-105 transform-gpu shadow-lg hover:shadow-xl backdrop-blur-sm",
                      isActive 
          ...
          {activeFiltersCount > 0 && (
            <div className="flex flex-wrap gap-3 p-4 bg-gradient-to-r from-blue-50/60 to-indigo-50/40 backdrop-blur-sm rounded-2xl border border-white/30 animate-scale-in">
              ...
              {Object.entries(filters).map(([key, value]) => {
                ...
                return (
                  <Badge 
                    key={key}
                    variant="secondary" 
                    className="flex items-center gap-2 bg-white/80 backdrop-blur-sm border border-white/40 text-slate-700 hover:bg-white transition-all duration-300 rounded-xl px-3 py-1.5 shadow-sm"
                  >
                    <IconComponent className="h-3.5 w-3.5" />
                    {key === 'search' ? `"${value}"` : value}
                    <X 
                      className="h-3.5 w-3.5 ml-1 opacity-70 hover:opacity-100 cursor-pointer hover:scale-110 transition-all duration-200" 
                      onClick={(e) => {
                        e.stopPropagation();
          ...
          <div className="flex flex-col lg:flex-row gap-4">
            <div className="relative flex-1">
              <CommandSearchInput 
                ...
                className="flex-1 shadow-lg transition-all duration-500 focus-within:shadow-2xl bg-white/80 backdrop-blur-sm border-white/40 rounded-xl"
              />
            </div>
                  ...
                  <TooltipTrigger asChild>
                    <div>
                      <Select value={filters.location} onValueChange={(value) => handleFilterChange('location', value)}>
                        <SelectTrigger 
                          className={cn(
                            "w-full transition-all duration-500 bg-white/80 backdrop-blur-sm border-white/40 rounded-xl shadow-lg hover:shadow-xl",
                            filters.location ? 'border-blue-500 ring-2 ring-blue-500/20 bg-blue-50/50' : ''
                          )}
                        >
                          <div className="flex items-center gap-2">
                    ...
                    <div>
                      <Select value={filters.teacher} onValueChange={(value) => handleFilterChange('teacher', value)}>
                        <SelectTrigger 
                          className={cn(
                            "w-full transition-all duration-500 bg-white/80 backdrop-blur-sm border-white/40 rounded-xl shadow-lg hover:shadow-xl",
                            filters.teacher ? 'border-blue-500 ring-2 ring-blue-500/20 bg-blue-50/50' : ''
                          )}
                        >
                          <div className="flex items-center gap-2">
                        ...
                        <SelectContent className="max-h-[300px] bg-white/95 backdrop-blur-xl border border-white/30 shadow-2xl rounded-xl">
                          ...
                          {teachers.map(teacher => (
                            <SelectItem key={teacher} value={teacher} className="rounded-lg">
                              {teacher}
                            </SelectItem>
                          ))}
                        </SelectContent>
                    ...
                    <div>
                      <Select value={filters.period} onValueChange={(value) => handleFilterChange('period', value)}>
                        <SelectTrigger 
                          className={cn(
                            "w-full transition-all duration-500 bg-white/80 backdrop-blur-sm border-white/40 rounded-xl shadow-lg hover:shadow-xl",
                            filters.period ? 'border-blue-500 ring-2 ring-blue-500/20 bg-blue-50/50' : ''
                          )}
                        >
                          <div className="flex items-center gap-2">
              ...
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button 
                      variant="outline" 
                      className="shrink-0 bg-gradient-to-r from-red-50 to-rose-50 hover:from-red-100 hover:to-rose-100 border-red-200 hover:border-red-300 text-red-600 transition-all duration-500 rounded-xl shadow-lg hover:shadow-xl transform hover:scale-105" 
                      onClick={handleReset}
                    >
