...
import { Table, LayoutGrid, List, BarChart, PieChart, Calendar, SlidersHorizontal, User, Store, MapPin, Eye, ArrowUpDown, Settings, Sparkles } from 'lucide-react';
...
const TableViewOptions: React.FC<TableViewOptionsProps> = ({
  ...
  return <div className="flex flex-col space-y-6 animate-fade-in">
          ...
          <div className="flex items-center gap-3 ml-6">
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" size="sm" className="h-10 gap-3 bg-white/80 hover:bg-white/90 backdrop-blur-md shadow-subtle border-white/40 hover:border-white/60 transition-all duration-300 hover:shadow-md hover:scale-[1.02] group">
                ...
                <div className="p-1 space-y-1">
                  <DropdownMenuSub>
                    <DropdownMenuSubTrigger className="hover:bg-gradient-to-r hover:from-slate-50/80 hover:to-white/90 transition-all duration-300 rounded-lg group">
                      ...
                      {activeGroupBy && activeGroupBy !== 'none' && <Badge variant="luxury" className="ml-auto animate-fade-in">
                    ...
                    <DropdownMenuPortal>
                      <DropdownMenuSubContent className="max-h-80 overflow-y-auto bg-white/95 backdrop-blur-xl border-white/40 shadow-luxury animate-scale-in rounded-xl">
                        <div className="p-2 space-y-1">
                          {availableColumns.map(column => <DropdownMenuCheckboxItem key={column} checked={visibleColumns.includes(column)} onCheckedChange={checked => handleColumnVisibilityChange(column, checked as boolean)} className="hover:bg-gradient-to-r hover:from-slate-50/60 hover:to-white/80 transition-all duration-300 rounded-lg font-medium">
                              {column}
                  ...
                  <DropdownMenuSub>
                    ...
                    <DropdownMenuPortal>
                      <DropdownMenuSubContent className="bg-white/95 backdrop-blur-xl border-white/40 shadow-luxury animate-scale-in rounded-xl">
                        <div className="p-2 space-y-1">
                          {availableColumns.map(column => <DropdownMenuSub key={column}>
                              <DropdownMenuSubTrigger className="hover:bg-gradient-to-r hover:from-slate-50/60 hover:to-white/80 justify-between transition-all duration-300 rounded-lg group">
                                ...
                                {activeSort.column === column && <Badge variant="luxury" className="ml-auto animate-fade-in">
                                    {activeSort.direction === 'asc' ? 'Asc' : 'Desc'}
                                  </Badge>}
                              </DropdownMenuSubTrigger>
                              <DropdownMenuPortal>
                                <DropdownMenuSubContent className="bg-white/95 backdrop-blur-xl border-white/40 shadow-luxury animate-scale-in rounded-xl">
                                  <DropdownMenuItem onClick={() => onSortChange(column, 'asc')} className="hover:bg-gradient-to-r hover:from-slate-50/60 hover:to-white/80 transition-all duration-300 rounded-lg group">
                                  ...
                                  <DropdownMenuItem onClick={() => onSortChange(column, 'desc')} className="hover:bg-gradient-to-r hover:from-slate-50/60 hover:to-white/80 transition-all duration-300 rounded-lg group">
