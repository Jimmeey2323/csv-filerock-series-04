...
const TableViewOptions: React.FC<TableViewOptionsProps> = ({
  ...
  const views = [
    ...
    { id: 'metrics', label: 'Metrics Grid', icon: LayoutGrid },
    { id: 'performance', label: 'Performance View', icon: TrendingUp },
    { id: 'analytics', label: 'Analytics Dashboard', icon: BarChart3 },
    { id: 'insights', label: 'Insights Panel', icon: PieChart },
  ...
  return (
    <div className="flex flex-col space-y-6 animate-fade-in">
      <div className="relative overflow-hidden rounded-2xl shadow-luxury border border-white/30">
        <div className="absolute inset-0 bg-gradient-to-br from-white/70 via-white/60 to-white/50 backdrop-blur-xl" />
        <div className="absolute inset-0 bg-gradient-to-r from-blue-500/3 via-purple-500/3 to-blue-500/3 animate-pulse-soft" />
        
        <div className="relative flex justify-between items-center p-4">
          <Tabs value={activeView} onValueChange={onViewChange} className="w-full">
            <TabsList className="w-full justify-start overflow-x-auto no-scrollbar bg-white/50 backdrop-blur-md border border-white/40 shadow-subtle">
              {views.map(view => (
                <TabsTrigger 
                  key={view.id} 
                  value={view.id} 
              ...
              <DropdownMenuTrigger asChild>
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-slate-700 font-medium">View Options</span>
                  </div>
                </Button>
              </DropdownMenuTrigger>
                      ...
                      <DropdownMenuSubContent className="bg-white/95 backdrop-blur-xl border-white/40 shadow-luxury animate-scale-in rounded-xl overflow-hidden">
                        <DropdownMenuRadioGroup value={activeGroupBy} onValueChange={handleGroupByChange}>
                          ...
                          <DropdownMenuRadioItem value="period" className="hover:bg-gradient-to-r hover:from-slate-50/60 hover:to-white/80 transition-all duration-300 rounded-lg group">
                            <Target className="mr-3 h-4 w-4 text-amber-500 transition-transform duration-300 group-hover:scale-110" />
